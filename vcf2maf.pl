...
## Adapted from https://github.com/ckandoth/vcf2maf
## Edited by SA to include TCGA MAF specifications
##
## Additional Input: Gene- Entrez ID map
####
...
my ( $tumor_uuid, $normal_uuid ) = ( "TUMOR_UUID", "NORMAL_UUID" );
my ( $disease_type) = ("-" );
my ( $vep_path, $vep_data, $vep_forks, $ref_fasta ) = ( "/export/cse/samreen/tools/vep", "/export/cse/samreen/tools/.vep", 4,
"/export/cse/samreen/hg19/Homo_sapiens_assembly19.fasta");
#"/export/cse/samreen/tools/.vep/homo_sapiens/79_GRCh38/Homo_sapiens.GRCh38.dna.primary_assembly.fa" );
my ( $snpeff_path, $snpeff_data, $snpeff_db ) = ( "/export/cse/samreen/tools/VarScan/snpEff", "/export/cse/samreen/tools/VarScan/snpEff/data", "GRCh37.75" );
my ( $ncbi_build, $maf_center, $min_hom_vaf ) = ( "GRCh37", "mdanderson.org", 0.7 );
...
if( $vep_anno ) {
    ...
    if( $use_snpeff ) {
        ...
        if( -s $snpeff_anno ) {
            ...
            my $snpeff_cmd = "java -Xmx4g -jar $snpeff_path/snpEff.jar -formatEff -config $snpeff_path/snpEff.config -dataDir $snpeff_data -noStats -sequenceOntology -hgvs $snpeff_db $input_vcf > $snpeff_anno";
...
my @maf_header = qw(
    ...
    Exon_Number t_depth t_ref_count t_alt_count n_depth n_ref_count n_alt_count all_effects Disease_Type
);
...
## Create the hashmap gene symbol Entrez ID
my %genetoID = ();
unless (open (INFILE, "/export/cse/samreen/LGG_GBM/DATABASE/Gene-ID.txt")){ die "Cant open input.txt: $!";}
while (<INFILE>){
    chomp;
    next unless (/\t/);
    my ($value, $key ) = split(/\t/, $_, 2);
    $genetoID{$key}=$value;
}
...
while( my $line = $vcf_fh->getline ) {
    ...
    if( exists $genetoID{$maf_line{Hugo_Symbol}}){
      $maf_line{Entrez_Gene_Id}  = $genetoID{$maf_line{Hugo_Symbol}};
    }
    ...
    $maf_line{Mutation_Status} = 'Somatic';
    $maf_line{Validation_Status} = 'Untested';
    $maf_line{Verification_Status} = 'Unknown';
    $maf_line{Validation_Method} = 'none';
    $maf_line{Sequence_Source} = 'WXS';
    $maf_line{Sequencer} = 'Illumina GAIIx';
    $maf_line{Disease_Type} = $disease_type;
    if( defined $tum_info{GT} and $tum_info{GT} ne "." and $tum_info{GT} ne "./." ) {
    ...
    $maf_line{Tumor_Sample_UUID} = $tumor_uuid;
    $maf_line{Matched_Norm_Sample_UUID} = $normal_uuid;
